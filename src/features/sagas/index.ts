...
function* fetchCampaignSaga(
  ...
  try {
    const {coordinates, startEnd} = action.payload;
    const strQuery = `?lat_sw=${coordinates.lat_sw}&lng_sw=${coordinates.lng_sw}&lat_ne=${coordinates.lat_ne}&lng_ne=${coordinates.lng_ne}&date_from=${startEnd[0]}&date_to=${startEnd[1]}`;
